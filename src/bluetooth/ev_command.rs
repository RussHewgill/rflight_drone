...
pub enum ReturnParameters {
    ...
    /// command.
    HalWriteConfigData(hci::Status<crate::bluetooth::events::Status>),
    ...
    /// command.
    HalSetTxPowerLevel(hci::Status<crate::bluetooth::events::Status>),

    /// Status returned by the [HAL Device Standby](crate::hal::Commands::device_standby) command.
    HalDeviceStandby(hci::Status<crate::bluetooth::events::Status>),
    ...
    HalStartTone(hci::Status<crate::bluetooth::events::Status>),

    /// Status returned by the [HAL Stop Tone](crate::hal::Commands::stop_tone) command.
    HalStopTone(hci::Status<crate::bluetooth::events::Status>),
    ...
    #[cfg(feature = "ms")]
    GattUpdateLongCharacteristicValue(hci::Status<crate::bluetooth::events::Status>),
    ...
    L2CapConnectionParameterUpdateResponse(hci::Status<crate::bluetooth::events::Status>),
}

impl hci::event::VendorReturnParameters for ReturnParameters {
    type Error = crate::bluetooth::events::BlueNRGError;
    ...
    fn new(bytes: &[u8]) -> Result<Self, hci::event::Error<Self::Error>> {
        ...
        match hci::Opcode(LittleEndian::read_u16(&bytes[1..])) {
            ...
            crate::bluetooth::opcode::GAP_PASS_KEY_RESPONSE => Ok(
                ReturnParameters::GapPassKeyResponse(to_status(&bytes[3..])?),
            ),
            crate::bluetooth::opcode::GAP_AUTHORIZATION_RESPONSE => Ok(
                ReturnParameters::GapAuthorizationResponse(to_status(&bytes[3..])?),
            ),
            crate::bluetooth::opcode::GAP_INIT => {
                Ok(ReturnParameters::GapInit(to_gap_init(&bytes[3..])?))
            }
            crate::bluetooth::opcode::GAP_SET_NONCONNECTABLE => Ok(
                ReturnParameters::GapSetNonConnectable(to_status(&bytes[3..])?),
            ),
            crate::bluetooth::opcode::GAP_SET_UNDIRECTED_CONNECTABLE => Ok(
                ReturnParameters::GapSetUndirectedConnectable(to_status(&bytes[3..])?),
            ),
            crate::bluetooth::opcode::GAP_UPDATE_ADVERTISING_DATA => Ok(
            ...
            crate::bluetooth::opcode::GAP_CONFIGURE_WHITE_LIST => Ok(
                ReturnParameters::GapConfigureWhiteList(to_status(&bytes[3..])?),
            ),
            crate::bluetooth::opcode::GAP_CLEAR_SECURITY_DATABASE => Ok(
                ReturnParameters::GapClearSecurityDatabase(to_status(&bytes[3..])?),
            ),
            crate::bluetooth::opcode::GAP_ALLOW_REBOND => {
                Ok(ReturnParameters::GapAllowRebond(to_status(&bytes[3..])?))
            }
            crate::bluetooth::opcode::GAP_TERMINATE_PROCEDURE => Ok(
                ReturnParameters::GapTerminateProcedure(to_status(&bytes[3..])?),
            ),
            crate::bluetooth::opcode::GAP_RESOLVE_PRIVATE_ADDRESS => {
                #[cfg(not(feature = "ms"))]
                {
            ...
            crate::bluetooth::opcode::GAP_START_OBSERVATION_PROCEDURE => {
                #[cfg(feature = "ms")]
                {
            ...
            crate::bluetooth::opcode::GAP_IS_DEVICE_BONDED => {
            ...
            crate::bluetooth::opcode::GATT_UPDATE_LONG_CHARACTERISTIC_VALUE => {
                #[cfg(feature = "ms")]
                {
...
fn check_len_at_least(
    buffer: &[u8],
    len: usize,
) -> Result<(), hci::event::Error<crate::bluetooth::events::BlueNRGError>> {
    if buffer.len() < len {
...
fn to_status(
    ...
    hci::Status<crate::bluetooth::events::Status>,
...
fn to_hal_config_data(
    bytes: &[u8],
) -> Result<HalConfigData, hci::event::Error<crate::bluetooth::events::BlueNRGError>> {
...
) -> Result<HalConfigParameter, hci::event::Error<crate::bluetooth::events::BlueNRGError>>
{
    match bytes.len() {
        ...
        other => Err(hci::event::Error::Vendor(
            crate::bluetooth::events::BlueNRGError::BadConfigParameterLength(other),
        )),
    }
}
...
impl TryFrom<u8> for LinkState {
    ...
    fn try_from(value: u8) -> Result<Self, Self::Error> {
        match value {
            ...
            _ => Err(crate::bluetooth::events::BlueNRGError::UnknownLinkState(
                value,
            )),
        }
    }
...
fn to_hal_link_status(
    bytes: &[u8],
) -> Result<HalLinkStatus, hci::event::Error<crate::bluetooth::events::BlueNRGError>> {
...
fn to_hal_anchor_period(
    bytes: &[u8],
) -> Result<HalAnchorPeriod, hci::event::Error<crate::bluetooth::events::BlueNRGError>> {
    require_len!(bytes, 9);
...
fn to_gap_init(
    bytes: &[u8],
) -> Result<GapInit, hci::event::Error<crate::bluetooth::events::BlueNRGError>> {
    require_len!(bytes, 7);
...
fn to_boolean(value: u8) -> Result<bool, crate::bluetooth::events::BlueNRGError> {
    match value {
        0 => Ok(false),
        1 => Ok(true),
        _ => Err(crate::bluetooth::events::BlueNRGError::BadBooleanValue(
            value,
        )),
    }
}
...
fn to_gap_security_level(
    bytes: &[u8],
) -> Result<GapSecurityLevel, hci::event::Error<crate::bluetooth::events::BlueNRGError>> {
    require_len!(bytes, 5);
...
fn to_gap_bonded_devices(
    bytes: &[u8],
) -> Result<GapBondedDevices, hci::event::Error<crate::bluetooth::events::BlueNRGError>> {
    let status = to_status(bytes)?;
    match status {
        hci::Status::Success => {
            ...
            if bytes.len() != HEADER_LEN + ADDR_LEN * address_count {
                return Err(hci::event::Error::Vendor(
                    crate::bluetooth::events::BlueNRGError::PartialBondedDeviceAddress,
                ));
            }
...
fn to_gatt_service(
    bytes: &[u8],
) -> Result<GattService, hci::event::Error<crate::bluetooth::events::BlueNRGError>> {
    require_len!(bytes, 3);
...
fn to_gatt_handle_value(
    bytes: &[u8],
) -> Result<GattHandleValue, hci::event::Error<crate::bluetooth::events::BlueNRGError>> {
